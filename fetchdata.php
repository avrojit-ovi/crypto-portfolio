            ...
            <div class="col-sm-2">
                <div id="inddiv" class="card text-white bg-info">
                    ...
                    <div class="card-body">
                        <center>
                            <!-- Update the innerHTML here -->
                            <div id="totalin" class="card-text "></div>
                        </center>
                    </div>
                </div>
            </div>
...
<?php
    ...
    if ($result->num_rows > 0) {
        echo "<div class='bd-example table-responsive'>";
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            ws_" . $row["id"] . ".onmessage = (event) => {
                ...
                if (price_" . $row["id"] . " > old_price_" . $row["id"] . ") {
                    ...
                    document.getElementById('coinPrice_" . $row["id"] . "').style.color = 'white';
                } else {
    ...
    <script>
        ...
        function calculateTotalPL() {
            ...
            cBalanceElements.forEach(
                (cBalanceElement, index) => { // Use distinct variable name for cBalanceElement
            ...
            document
                .getElementById('totalin')
                .textContent = `$ ${totalInvestment.toFixed(2)}`;

            // Print the current balance

            document
                .getElementById('cBalance')
            ...
            if (allPricesDisplayed) {
                ...
                document
                    .getElementById('totalPL')
                    .textContent = totalPLText;

                const totalPLPercentText = `${totalChange.toFixed(2)}%`;
                document
                    .getElementById('totalPLPercent')
                    .textContent = totalPLPercentText;
                ...
                changeElements.forEach((changeElement) => {
                    ...
                    if (parseFloat(changeElement.innerText) >= 0) {
                        ...
                        document
                            .getElementById('plmdiv')
                            .className = 'card text-white bg-success';
                        document
                            .getElementById('plddiv')
                            .className = 'card text-white bg-success';
                        document
                            .getElementById('cBalancediv')
                            .className = 'card border-success text-success';
                        document
                            .getElementById('avrppnldiv')
                            .className = 'card border-success text-success';
                    } else {
                        row
                            .classList
                ...
                document
                    .getElementById('plmdiv')
                    .className = 'card text-white bg-info'; // Reset the class to default
                document
                    .getElementById('plddiv')
                    .className = 'card text-white bg-info'; // Reset the class to default
                document
                    .getElementById('cBalancediv')
                    .className = 'card border-info text-white'; // Reset the class to default
                document
                    .getElementById('avrppnldiv')
                    .className = 'card border-info text-white'; // Reset the class to default
                const changeElements = document.querySelectorAll('td[id^="change_"]');
